...
import { StorageUtils } from "./supabase.js";

// ==================== Schedule History Management ==================== //

async function populateSavedSchedules() {
  const savedSchedules = await getSavedSchedulesFromStorage();
...
async function getSavedSchedulesFromStorage() {
  try {
    return await StorageUtils.getSchedules();
  } catch (error) {
...
async function saveSchedulesToStorage(schedules) {
  try {
    // Save individual schedules to database
    for (const schedule of schedules) {
      await StorageUtils.saveSchedule({
        name: schedule.name,
        employee_data: schedule.employeeData,
        schedule_data: schedule.scheduleData,
        leave_requests: schedule.leaveData || {},
...
async function loadSelectedSchedule() {
  ...
  showSpinner();
  try {
    await loadScheduleFromStorage(scheduleName);
  } finally {
    hideSpinner();
  }
}
...
async function loadScheduleFromStorage(scheduleName) {
  const savedSchedules = await getSavedSchedulesFromStorage();
...
async function deleteSelectedSchedule() {
  ...
  if (confirmScheduleDeletion(scheduleName)) {
    await removeScheduleFromStorage(scheduleName);
    await populateSavedSchedules();
...
async function removeScheduleFromStorage(scheduleName) {
  try {
    await StorageUtils.deleteSchedule(scheduleName);
  } catch (error) {
    console.warn("Database unavailable, falling back to localStorage:", error);
    let savedSchedules =
