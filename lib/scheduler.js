...
async function loadEmployees() {
  try {
    const employees = await StorageUtils.getEmployees();
...
async function saveEmployees() {
  try {
    for (const employee of appState.employees) {
      await StorageUtils.saveEmployee(employee);
    }
  } catch (error) {
    console.warn("Database unavailable, falling back to localStorage:", error);
...
async function saveCurrentSchedule() {
  ...
  try {
    ...
    await StorageUtils.saveSchedule(scheduleData);
    await populateSavedSchedules();
...
async function loadSelectedSchedule() {
  ...
  try {
    const scheduleData = await StorageUtils.loadSchedule(scheduleId);
    if (scheduleData) {
...
async function deleteSelectedSchedule() {
  ...
  if (confirm("Are you sure you want to delete this schedule?")) {
    try {
      await StorageUtils.deleteSchedule(scheduleId);
      await populateSavedSchedules();
...
function attachButtonEventListeners() {
  ...
  if (closeLeaveModal) {
    closeLeaveModal.addEventListener("click", closeLeaveModal);
  }
  ...
  if (closeClearModal) {
    closeClearModal.addEventListener("click", closeClearModal);
  }
